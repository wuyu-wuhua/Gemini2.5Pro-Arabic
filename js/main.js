...
function getSafeTranslations() {
    ...
    return {
        ...
        ar: { 'untitled-chat': 'Без названия чата', 'history-title': 'История чата', 'no-history': 'Еще нет истории', 'welcome-message': 'Привет! Чем могу помочь?' }
    };
}
...
function switchLanguage(lang) {
    if (lang && lang === 'ar') { // Only ar is supported now
...
function applyTranslations(languageToApply) {
    const currentLang = languageToApply || localStorage.getItem('language') || 'ar';
    const trans = getSafeTranslations();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    langOptions.forEach(option => {
        ...
        // Since only ar is supported, ensure only ar option can be active if it exists
        if (option.getAttribute('data-lang') !== 'ar') option.style.display = 'none'; else option.style.display = 'block';
    });
...
if (langOptions) {
    langOptions.forEach(option => {
        ...
        newOption.addEventListener('click', (e) => {
            ...
            if (lang === 'ar') { // Only allow switching to ar
                switchLanguage(lang);
            }
        });
    });
